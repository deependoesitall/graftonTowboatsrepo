...
**FIFTH BUILD PASS (July 19, late night) — FULL-STORE IMPORT SHIPPED. ROUND 8 P1 = 8/8 COMPLETE.**
Live API probe findings: Freshop catalog ballooned to 71,646 rows (AWG warehouse superset); the BROWSABLE store = storefront department tree (`department_id=21585437`) = **20,771 items**; department paths in canonical_url drive category mapping + alcohol exclusion (whole beer_wine_spirits subtree dropped). Built: migration `037_full_store_import.sql` (products.store_only default FALSE + get_category_counts scoped to barge catalog); sync lib scoped to storefront tree (~208 pages, not 717) + buildStoreProduct (category/sub_category from dept path, weighable via is_weight_required OR price-embedded-UPC rule, uom LB/EA, store_only=true); nightly cron now IMPORTS unmatched items as store products (dedup by UPC keys, ~19k inserts on first sweep — may span two nights, session checkpoint carries across days now); catalog UI: barge form = default view, teal "browse the full store" expander scoped to active category/search with live match counts, full-store banner + back link, store param preserved across pagination/category clicks (via props, not window — no hydration regressions); checkout step 1 "Shop the rest of the store" card; admin Barge/Store filter dropdown + /api/products store param (side bonus: admin add-item + substitution search now draws from the whole store); form-layout matching scoped to barge catalog (desc matching can't misfire on 20k store items).
⚠ Deploy: apply **036 + 037** before deploying. First full sweep: trigger the GH workflow repeatedly or let it run overnight (~14–20 chunks).
Remaining cleanup: fix the 17 unmatched form rows · confirm coupons gone after cache · P2 list.
...
- **Safety rails:** normalize UPCs + validate check digits before rendering; items with bad/missing UPCs are flagged (printed without a barcode) never mis-encoded. ~~Go/no-go 10-item test~~ **Deepen waived the sample test (July 19)** — encoding verified against 3 ground-truth Freshop barcodes + UPC-A invariants; bars printed larger (~0.4mm × 12mm) for first-scan reliability on any printer. Fallback if a gun ever balks: the UPC digits print under every barcode for manual keying.
