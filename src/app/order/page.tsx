...
export default function OrderPage() {
  ...
  const setPay = (name: string, patch: Partial<CodPay>) =>
    setCodPay(prev => {
      const next = { ...prev, [name]: { ...(prev[name] ?? { method: '', handle: '', phone: '', time: '' }), ...patch } };
      // Persisted so Repeat Order can bring it back, and so a refresh mid-order
      // doesn't lose it. Same reasoning as the vessel block.
      saveCodPayments(next);
      return next;
    });
  ...
  useEffect(() => {
    ...
    // Per-person COD payment — from a Repeat Order, or from this same order
    // before a refresh. Names no longer on any COD line are simply never read.
    setCodPay(getCodPayments());
    setItems(getCart());
