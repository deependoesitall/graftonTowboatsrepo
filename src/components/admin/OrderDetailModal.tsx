...
import { X, Download, FileText, Printer, Trash2, Loader2, ShoppingCart } from 'lucide-react';
                    ...
                    {order.items.map(item => (
                      <tr key={item.id} className={item.shopping_status === 'out_of_stock' ? 'opacity-40 line-through' : ''}>
                        ...
                        <td className="px-3 py-2 text-right font-bold text-xs">
                          {formatCurrency(item.actual_total ?? item.unit_price * (quantities[item.id] ?? item.quantity))}
                        </td>
                      </tr>
                    ))}
                  </tbody>
