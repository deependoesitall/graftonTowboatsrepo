...
function itemCard(
  ...
  // SCAN FIRST: densify yielded. moduleWidth 3 / height 60 / CSS matches intrinsic (no downscale).
  // 3-col grid so flex cannot crush bars. flex-shrink:0 + quiet pad.
  const svg = weighable ? null : upcASvg(i.upc, { moduleWidth: 3, height: 60 });
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* LANDSCAPE. Dave: more barcodes across the page. Four cards per row —
     3 cards/row so barcodes keep moduleWidth 3 without CSS crush. */
  ...
  .grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 3px; padding: 2px 0; }
  ...
  /* Never CSS-scale barcodes down — that thins modules and kills handheld scans. */
  .bc { flex: 0 0 auto; padding: 2px 6px; background: #fff; overflow: visible; }
  .bc svg { height: auto; width: auto; max-width: none; display: block; }
